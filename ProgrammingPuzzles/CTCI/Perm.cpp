...
int Permute();
int PermuteRec(int array[], int length, int index, int base, int count);
...
int Permute(int arr[], int length){
	...
	for (int i = 0; i < N; i++){
		PermuteRec(permuteArr, length, i, i,0);
	}
	return 0;
}
...
int PermuteRec(int array[], int length, int index, int base, int count){
	...
	if (length <= 0 || index < 0 || base < 0){ return 0; }
	...
	for (leftDir = index - 1; leftDir >= 0; leftDir--){
		PermuteRec(array, length, leftDir, index,  count +1);
	}

	for (rightDir= index + 1; rightDir <N; rightDir++){
		PermuteRec(array, length, rightDir, index, count + 1);
	}
