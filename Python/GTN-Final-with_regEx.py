...
def range100():
    ...
    # assignments of global variables
    maxRange = 100
    current_game = 0
    ...
    # Secrete number for computer guess
...
def input_guess(guess):
    ...
    #let user add any guess once game has been choosen
    if ( not guess ) or (not guess.strip(" ")):
        print "Invalid guess!"
        return
    
    #also make sure that it contains only valid integral numbers
    pattern = r'[^0-9]'
    if re.search(pattern, guess):
        #Character other then 0-9 was found
        print 'Invalid : %r' % (guess,)
        inputField.set_text("")
        return
    else:
        #No character other then 0-9 was found
        print 'Valid   : %r' % (guess,)
    ...
    if ( player_guess < comp_guess):
        guess_count -= 1
        # Check for guess_count reached allowed_guesses,if so restart game
        if ( guess_count >= 1 ):
